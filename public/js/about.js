...
const setupMovieInfo = (data) => {
  ...
  title.innerHTML = movieName = data.title;
  ...
  backdrop.style.backgroundImage =
    "url(${original_img_url}${data.backdrop_path})";
};
...
fetch(
  ...
  .then((data) => {
    ...
    for (let i = 0; i < maxClips; i++) {
      trailerContainer.innerHTML += `<iframe src="https://youtube.com/embed/${data.results[i].key}"
...
.then(data => {
    ...
    container.innerHTML += `
    <div class="movie" onclick="loaction.href = '/${data.results[i].id}'">
        <img src="${img_url}${data.results[i].backdrop_path}" alt="poster#1">
